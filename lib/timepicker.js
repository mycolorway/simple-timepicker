...
Timepicker = (function(_super) {
  ...
  Timepicker.prototype._render = function() {
    ...
    if (this.opts.inline) {
      ...
      return $(document).on('click.simple-timepicker', (function(_this) {
        return function(e) {
          if ($(e.target).is(_this.target)) {
            return;
          }
  ...
  Timepicker.prototype._renderTime = function() {
    var meridiem;
    meridiem = this._t(this.time.format('a'));
    this.el.find('.time').text(meridiem + ' ' + this.time.format(this._t('time-format')));
