...
Timepicker = (function(_super) {
  ...
  Timepicker.addView = function(view) {
    if (!this.views) {
      this.views = [];
    }
...
Hourview = (function(_super) {
  ...
  Hourview.prototype._inputTpl = '<input typt="text" class="view-input hour-input" data-type="minute" />';

  Hourview.prototype.isAm = true;
  ...
  Hourview.prototype._renderPanel = function() {
    ...
    el = "<div class='panel panel-hour' data-meridiem=" + (this.isAm ? 'am' : 'pm') + ">";
    el += '<a class="menu-item" data-action="am">AM</a><a class="menu-item" data-action="pm">PM</a>';
  ...
  Hourview.prototype._handleAction = function(action) {
    this.isAm = action === 'am' ? true : false;
    if (this.isAm) {
      return this.panel.attr('data-meridiem', 'am');
    } else {
      return this.panel.attr('data-meridiem', 'pm');
    }
  };
  ...
  Hourview.prototype._onDateChangeHandler = function(e) {
    this.value = e.minute;
    this.isAm = this.value > 12 ? false : true;
    this._refreshInput();
    this._refreshSelected();
    return this._handleAction(this.isAM ? 'am' : 'pm');
  };
...
MinuteView = (function(_super) {
  ...
  MinuteView.prototype.opts = {
    forceFormat: true
  };

  MinuteView.prototype._inputTpl = '<input typt="text" class="view-input minute-input" data-type="minute" />';

  MinuteView.prototype._renderPanel = function() {
    var el, minute, _i;
    if (this.opts.forceFormat) {
      this.value = Math.floor(Number(this.value) / 5) * 5;
    }
  ...
  MinuteView.prototype._handleAction = function(action) {
    this.isAm = action === 'am' ? true : false;
    if (this.isAm) {
      return this.panel.attr('data-meridiem', 'am');
    } else {
  ...
  MinuteView.prototype._onInputHandler = function(e) {
    ...
    if (value.length === 2 && Number(value) < 60) {
      if (this.opts.forceFormat) {
        value = Math.floor(Number(value) / 5) * 5;
      }
      return this.select(value, true, true);
    } else if (value.length === 1) {
      return this.timer = setTimeout((function(_this) {
